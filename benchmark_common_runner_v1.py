...
def infer_top_label_and_fraction_from_csv(path: Path) -> Tuple[Optional[str], Optional[float]]:
    ...
    label_candidates = ["state", "stage", "label", "name", "cell_state", "sample_week"]
    frac_candidates = ["fraction", "fraction_within_query", "proportion", "frac", "share"]
    count_candidates = ["n", "n_cells", "count"]

    label_col = next((c for c in label_candidates if c in df.columns), None)
    frac_col = next((c for c in frac_candidates if c in df.columns), None)
    ...
    if frac_col is not None:
        tmp = df.sort_values(frac_col, ascending=False).iloc[0]
        return str(tmp[label_col]), float(tmp[frac_col])
    if count_col is not None:
        tmp = df.sort_values(count_col, ascending=False).iloc[0]
        total = float(df[count_col].sum()) if float(df[count_col].sum()) != 0 else None
        frac = float(tmp[count_col]) / total if total else None
...
def build_key_metrics_row(
    ...
    top_stage_label, top_stage_fraction = infer_top_label_and_fraction_from_csv(stage_csv_path)
    top_state_label, top_state_fraction = infer_top_label_and_fraction_from_csv(state_csv_path)

    def pick_number(*values: Any) -> Any:
        for v in values:
            if v is None:
                continue
            if str(v) == "nan":
    ...
    row = {
        ...
        "n_query_cells": pick_number(
            whole.get("n_query_cells"), epi.get("n_query_cells"), whole.get("n_cells_total")
        ),
        "n_query_epi_eligible": pick_number(
            whole.get("n_query_epi_eligible"), epi.get("n_query_epi_eligible")
        ),
        "lineage_off_target_fraction": pick_number(
            whole.get("lineage_off_target_fraction"), epi.get("lineage_off_target_fraction")
        ),
        "top_stage": pick_number(
            whole.get("top_stage"), epi.get("top_stage"), top_stage_label
        ),
        "top_stage_fraction": pick_number(
            whole.get("top_stage_fraction"), epi.get("top_stage_fraction"), top_stage_fraction
        ),
        "top_state": pick_number(
            epi.get("top_state"), whole.get("top_state"), top_state_label
        ),
        "top_state_fraction": pick_number(
            epi.get("top_state_fraction"), whole.get("top_state_fraction"), top_state_fraction
        ),
