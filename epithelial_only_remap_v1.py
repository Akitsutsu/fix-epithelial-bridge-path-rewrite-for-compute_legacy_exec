...
QUERY_H5AD = "converted/query_CA1_clean.h5ad"
WHOLELUNG_GATE_CSV = "prototype_out_v1/CA1_cell_projection_v1.csv"
OUTDIR = "prototype_out_epi_v1"
...
def main():
    ...
    epi_pred["epi_input_eligible"] = eligible_mask
    epi_pred["lineage_off_target"] = ~eligible_mask
    ...
    # Write outputs
    # -------------------------
    merged.to_csv(outdir / "CA1_lineage_gate_plus_epi_remap.csv", index=False)
    ...
    epi_cells.to_csv(outdir / "CA1_epi_cell_projection.csv", index=False)
    ...
    value_counts_fraction(ambiguous, "epi_state_fine", out_name="state_fine").to_csv(
        outdir / "CA1_epi_ambiguous_state_fine.csv", index=False
    )
    value_counts_fraction(ambiguous, "epi_stage_fine", order=stage_order, out_name="stage_fine").to_csv(
        outdir / "CA1_epi_ambiguous_stage_fine.csv", index=False
    )
    ...
    value_counts_fraction(lineage_off, "lineage_gate_state_coarse", order=coarse_order, out_name="state_coarse").to_csv(
        outdir / "CA1_lineage_off_target_state_coarse.csv", index=False
    )
    ...
    with open(outdir / "CA1_epi_summary_v1.json", "w") as f:
